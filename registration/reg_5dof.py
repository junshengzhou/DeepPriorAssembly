...
class PointCloudRegistrationModel_5dof(nn.Module):
    def __init__(self):
        super(PointCloudRegistrationModel_5dof, self).__init__()
    ...
    def forward(self, source_points):
        cos_theta = torch.cos(self.rotation_y)
        sin_theta = torch.sin(self.rotation_y)

        rotation_matrix_y = torch.zeros(3, 3, device="cuda")
        rotation_matrix_y[0, 0] = cos_theta
        rotation_matrix_y[0, 2] = sin_theta
        rotation_matrix_y[2, 0] = -sin_theta
        rotation_matrix_y[2, 2] = cos_theta
        rotation_matrix_y[1, 1] = 1

        transformed_points = torch.matmul(source_points, rotation_matrix_y) + self.translation
...
def register_mesh(ind, target_points, gen_mesh_path, intri, exp_name):
    ...
    mesh.rotate(rotation_matrix_pre, center=(0, 0, 0))

    vertices = mesh.vertices
    vertices = o3d.utility.Vector3dVector(np.array(vertices) * [1, -1, 1])
    ...
    for rate in range(2):
        ...
        model = PointCloudRegistrationModel_5dof().cuda()
        ...
        for epoch in range(num_epochs):
            ...
            if epoch < 600:
                ...
                loss = loss_3d * 100 + loss_2dmatching
